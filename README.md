...
(Much better performance can be achieved, [0.725](https://github.com/shaofengzeng/SuperPoint-Pytorch/issues/6), if using the data generated by rpautrat's magicpoint. Our magicpoint generally produces more points, this may be the reason for the low accuracy. so setting proper values for `det_threshold`, `nms` and `top_k` may help you achieve better performance. )
